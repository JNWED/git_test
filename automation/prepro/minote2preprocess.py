...
class MiNote2PreProcess (BaseDevicePreProcess):

    def __init__(self, tester):
        super(MiNote2PreProcess, self).__init__(tester)
    ...
    def pre_process(self):
        ...
        email_login = self.tester.find_element_by_id('com.netease.cloudmusic:id/mailLabel', 10)
        if login_or_register or use_now or email_login:
            if(login_or_register):
                login_or_register.click()
                #need to check again!
                email_login = self.tester.find_element_by_id('com.netease.cloudmusic:id/mail', 10)
            if(email_login):
                email_login.click()
                # app站内登录--Email登录
